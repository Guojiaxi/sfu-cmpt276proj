...
module.exports = function(app) {
    ...
    app.get('/meetup', function(req, res, next) {
        ...
        var meetupQuery = ('SELECT * FROM meetup JOIN login ON (uid = ' + req.user + ') WHERE ' + req.user + '= ANY(users) ORDER BY mid');
        try {
            //Set return values for success and failure
    ...
    app.post('/meetup_cancel', (req, res) => {
        ...
        var cancelQuery = 'UPDATE meetup SET is_cancelled = ' + is_cancelled + ' where mid = ' + mid + " ORDER BY mid";
    ...
    app.post('/meetup_remove', (req, res) => {
        var mid = req.body.mid;

        //View a row's from person table
        var removeQuery = 'DELETE FROM meetup WHERE mid = ' + mid;

        try {
            pool.query(removeQuery, (error, result) => {
                if (error) {
                    res.redirect('/meetup_none');
                } else {
                    res.redirect('/meetup');
    ...
    app.get('/burnaby_map', (req, res) => {
        try {
            var eventsTodayQuery = "SELECT name, host_name, url, location, " +
            "TO_CHAR(start_datetime, 'HH24:MI') start_time, " +
            "TO_CHAR(end_datetime, 'Mon DD') end_date, " +
            "TO_CHAR(end_datetime, 'HH24:MI') end_time " +
            "FROM event WHERE TO_DATE(TO_CHAR(start_datetime, 'Mon DD'), 'Mon DD') = CURRENT_DATE " +
            "ORDER BY start_datetime ASC, name ASC";

            pool.query(eventsTodayQuery, (error, result) => {
                if (error) {
                    res.end('error');
                }
                var results = { 'results': result.rows };
                console.log("Sending Today's Events...");
                res.render('burnaby_map', results);
            });
        } catch (e) {
            res.redirect('home_none');
        }
